...
```{r}
ca2004 <- data.table::fread("/Users/Hope/OneDrive/Documents/20 Fall/PM566/HW/HW1/cadata2004.csv")
ca2019 <- data.table::fread("/Users/Hope/OneDrive/Documents/20 Fall/PM566/HW/HW1/cadata2019.csv")
```
...
```{r}
yr.pal=colorNumeric(c('yellow','red'), domain = pm$Year)
leaflet(pm) %>%
...
### County level
```{r}
pm_c <- group_by(pm2, Year, COUNTY_CODE) %>% 
  summarise(PM = mean(PM, na.rm = TRUE))
summary(pm_c$PM)
pm_c=mutate(pm_c, xyear=as.numeric(as.character(Year))) 
ggplot(pm_c)+
  geom_line(mapping = aes(x=xyear, y=PM, color=factor(COUNTY_CODE)))
```
